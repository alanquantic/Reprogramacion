...
const HistoryItem: React.FC<HistoryItemProps> = memo(({ item, onView, onDelete, deleteLabel }) => {
    ...
    if (history.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[calc(100vh-80px)] text-center px-4 animate-fade-in">
                <h2 className="text-3xl font-bold mb-4 text-purple-600 dark:text-purple-300">{t.historyEmpty}</h2>
    ...
    return (
        <div className="p-4 md:p-8 max-w-6xl mx-auto animate-fade-in">
            <div className="text-center mb-12">
                <h2 className="text-4xl font-bold mb-2">{t.historyTitle}</h2>
                <p className="text-gray-600 dark:text-gray-400">{t.historyDescription}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {history.map((item) => (
                    <HistoryItem
                        ...
                        deleteLabel={t.deleteSymbolLabel}
                    />
                ))}
            </div>
        </div>
