...
const ShareButton: React.FC<ShareButtonProps> = memo(({ image }) => {
    ...
    const handleDownloadImage = useCallback(async () => {
        try {
            ...
            link.href = image.url;
            const filename = `${language === 'es' ? 'simbolo-poder' : 'power-symbol'}-${image.scenarioTitle.toLowerCase().replace(/[^a-z0-9]/g, '-')}.jpeg`;
    ...
    const handleCopyAffirmation = useCallback(async () => {
        try {
            const text = language === 'es'
                ...
                : `"${image.affirmation}"\n\n- My power symbol: ${image.scenarioTitle}`;
    ...
    const handleNativeShare = useCallback(async () => {
        ...
        try {
            ...
            const filename = language === 'es' ? 'simbolo-poder.jpg' : 'power-symbol.jpg';
            const file = new File([blob], filename, { type: 'image/jpeg' });

            await navigator.share({
                title: `${t.shareTitle}: ${image.scenarioTitle}`,
    ...
    const handleShareTwitter = useCallback(() => {
        const hashtags = language === 'es' 
            ? '#ReprogramacionVisual #Meditacion' 
            : '#VisualReprogramming #Meditation';
        const text = encodeURIComponent(`"${image.affirmation}"\n\n🧘 ${t.shareTitle}: ${image.scenarioTitle.toLowerCase()}\n\n${hashtags}`);
    ...
    return (
        <div className="relative">
            ...
            >
                ...
                {isSharing ? t.shareSharing : t.shareButton}
            </button>
                ...
                <>
                    ...
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 p-2 w-56 z-50 animate-fade-in">
                        ...
                        >
                            ...
                            <span>{copySuccess ? t.shareCopied : t.shareCopyAffirmation}</span>
                        </button>
