...
import { LOADING_STEP_KEYS, getLoadingStepProgress } from '../constants';
...
const LoadingScreen: React.FC<LoadingScreenProps> = ({ currentStep }) => {
    const { t } = useLanguage();
    const steps = Object.keys(LOADING_STEP_KEYS) as (keyof typeof LOADING_STEP_KEYS)[];
    const currentStepIndex = steps.indexOf(currentStep ?? 'prompt');
    const progress = getLoadingStepProgress(currentStep ?? 'prompt');
            ...
            <div className="w-full max-w-md">
                <ul className="space-y-4">
                    {steps.map((stepKey, index) => {
                        const stepConfig = LOADING_STEP_KEYS[stepKey];
                        const stepText = t[stepConfig.textKey];
