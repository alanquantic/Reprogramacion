...
const SectionHeader: React.FC<{ children: React.ReactNode; className?: string }> = memo(({ children, className = '' }) => (
...
const ResultDisplay: React.FC<ResultDisplayProps> = memo(({ image, onBack, isFromHistory, onEditImage, isEditingImage, dispatch }) => {
    ...
    const optimizedImageUrl = useMemo(() => {
        if (image.url.startsWith('data:')) {
            return base64ToBlobUrl(image.url);
        }
        return image.url;
    }, [image.url]);
    ...
    return (
        <div className="p-4 md:p-8 max-w-3xl mx-auto animate-fade-in text-center">
            ...
            <div className="flex flex-col items-center gap-8">
                {/* Image - using optimized blob URL */}
