...
const InputForm: React.FC<InputFormProps> = ({ state, dispatch, onGenerate, onGenerateCustom }) => {
    ...
    const AreaStep = () => (
        <div className="p-4 md:p-8 text-center max-w-4xl mx-auto animate-slide-in-from-right">
            ...
            {lastSelections.areaId && <div className="mb-6 bg-purple-100 dark:bg-purple-900/30 p-3 rounded-lg border border-purple-300 dark:border-purple-500/30 max-w-md mx-auto animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {AREAS.map((areaInfo) => {
                    ...
                    let stateClasses = isLastSelected ? `${areaInfo.borderColor} ring-2 ${areaInfo.ringColor} shadow-md ${areaInfo.shadowColor}` : `border-gray-300 dark:border-purple-500/30 ${areaInfo.hoverBorderColor} hover:-translate-y-1`;
                    return (
                        <button
                            key={areaInfo.id}
                        ...
                        >
                            <div className={`mb-4 text-purple-500 dark:text-purple-300`}><IconRenderer iconId={areaInfo.iconId} /></div>
    ...
    const ScenarioStep = () => (
        <div className="p-4 md:p-8 max-w-4xl mx-auto animate-slide-in-from-right">
             <button onClick={() => dispatch({ type: 'GO_TO_STEP', payload: 'area' })} className="text-purple-600 dark:text-purple-300 hover:text-purple-800 dark:hover:text-purple-100 mb-6 text-sm">
                &larr; Volver a seleccionar área
            </button>
            <h2 className="text-3xl font-bold mb-2 text-center">Paso 2: Elige tu Intención</h2>
            <p className="text-gray-500 dark:text-gray-400 mb-8 text-center">
                Selecciona un escenario predefinido o crea el tuyo para el área de <span className="font-bold text-purple-600 dark:text-purple-300">{area}</span>.
            </p>

            <div className="mb-8">
                <h3 className="text-center text-lg font-semibold text-gray-700 dark:text-gray-300 mb-3">¿Cómo prefieres que me dirija a ti?</h3>
            ...
            {isCustomizing ? (
                <div className="my-8 p-6 bg-purple-100 dark:bg-gray-800/50 rounded-xl border-2 border-purple-300 dark:border-purple-500/50 animate-fade-in">
                    ...
                    <p className="text-sm text-center text-gray-600 dark:text-gray-400 mb-4">Describe la transformación que buscas. Sé específico y emocional. <br />Ej: "Liberar el miedo a hablar en público y sentirme seguro y elocuente".</p>
                    <textarea
                        value={customPrompt}
                        onChange={(e) => setCustomPrompt(e.target.value)}
                        className="w-full h-24 p-3 bg-white dark:bg-gray-700 border-2 border-gray-300 dark:border-purple-500/50 rounded-lg focus:ring-2 focus:ring-purple-500 dark:focus:ring-purple-400 focus:border-purple-500 dark:focus:border-purple-400 outline-none transition-colors"
                        placeholder="Escribe tu intención aquí..."
                    />
                    <div className="flex flex-col sm:flex-row justify-center items-center gap-4 mt-4">
                        <button onClick={handleCustomGenerate} disabled={!customPrompt.trim()} className="w-full sm:w-auto bg-purple-600 hover:bg-purple-700 text-white font-bold py-2 px-6 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                            Generar Símbolo Personalizado
                        </button>
                        <button onClick={() => setIsCustomizing(false)} className="w-full sm:w-auto text-sm text-purple-600 hover:text-purple-800 dark:text-purple-300 dark:hover:text-purple-100 transition-colors">
                ...
                <div className="mt-8">
                    ...
                    <div className="text-center mt-8">
                        <button onClick={() => setIsCustomizing(true)} className="text-purple-600 dark:text-purple-300 hover:text-purple-800 dark:hover:text-purple-100 font-semibold transition-colors">
