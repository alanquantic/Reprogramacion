...
// Isolated Name Step Component - completely separate from other steps
...
const NameStep: React.FC<NameStepProps> = memo(({ initialName, dispatch, t }) => {
    ...
    return (
        <div className="p-4 md:p-8 text-center max-w-lg mx-auto animate-slide-in-from-right">
            <div className="mb-8">
            ...
            <div className="mb-8">
                <input
                    ...
                    autoFocus
                    className={`w-full px-6 py-4 text-xl rounded-xl border-2 bg-white dark:bg-gray-800/50 text-gray-900 dark:text-white focus:border-purple-500 dark:focus:border-purple-400 focus:ring-2 focus:ring-purple-300 dark:focus:ring-purple-500/30 outline-none transition-all text-center ${
                        isValid ? 'border-green-400 dark:border-green-500/50' : 'border-gray-300 dark:border-purple-500/30'
                    }`}
                />
            </div>
            ...
            <button
                ...
                disabled={!isValid}
                className="bg-purple-600 hover:bg-purple-700 text-white font-bold py-3 px-10 rounded-full text-lg transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
            >
...
interface AreaStepProps {
    dispatch: React.Dispatch<AppAction>;
    lastSelections: { areaId: ReprogramArea | null; scenarioId: string | null };
    t: Translations;
}

const AreaStep: React.FC<AreaStepProps> = memo(({ dispatch, lastSelections, t }) => {
    ...
    const handleAreaSelect = useCallback((selectedArea: ReprogramArea) => {
        dispatch({ type: 'SELECT_AREA', payload: selectedArea });
    }, [dispatch]);

    const handleBack = useCallback(() => {
        dispatch({ type: 'GO_TO_STEP', payload: 'name' });
    }, [dispatch]);
    ...
    return (
        <div className="p-4 md:p-8 text-center max-w-4xl mx-auto animate-slide-in-from-right">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">{t.step1Description}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {AREAS.map((areaInfo) => {
                    ...
                    return (
                        <button
                            key={areaInfo.id}
                            onClick={() => handleAreaSelect(areaInfo.id)}
...
const InputForm: React.FC<InputFormProps> = ({ state, dispatch, onGenerate, onGenerateCustom }) => {
    ...
    const { area, gender, userName } = userInput;
    ...
    // Name Step - completely isolated
    if (formStep === 'name') {
        return <NameStep initialName={userName} dispatch={dispatch} t={t} />;
    }

    // Area Step - isolated component
    if (formStep === 'area') {
        return <AreaStep dispatch={dispatch} lastSelections={lastSelections} t={t} />;
    }
